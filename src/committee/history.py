...
from committee.agents import AGENTS, transfers_of
...
def build_agent_histories(
    ...
    for gw in range(1, up_to_gw):
        ...
        for agent, s in suggestions.items():
            ...
            earned = sum(points.get(pid, 0) for _, pid in pairs) + points.get(s["captain"], 0)
            picked = entry is not None and entry["picked"] == agent

            def name(pid: int) -> str:
                return names.get(pid, str(pid))

            moves = ", ".join(f"OUT {name(o)} IN {name(i)}" for o, i in pairs) or "no transfer"
            chip = f" with chip {s['chip']}" if s.get("chip") else ""
            per_agent[agent].append(
                f"GW{gw}: you advised {moves}{chip}, captain {name(s['captain'])}. "
